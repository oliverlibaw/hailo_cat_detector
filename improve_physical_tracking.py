#!/usr/bin/env python3
"""
improve_physical_tracking.py
A script to improve the physical tracking of detected objects with smoother movements.
Focuses on keeping detected objects centered in the frame without triggering the squirt gun.
"""
...
try:
    ...
    RELAY_PINS = {
        'squirt': 5,  # Squirt relay (not used in this script)
...
def setup_gpio():
    ...
    try:
        ...
        for name, pin in RELAY_PINS.items():
            if name != 'squirt':  # Skip squirt relay
...
def activate_relay(pin, duration):
    ...
    try:
        ...
        last_action = f"Relay {pin} ON ({duration:.3f}s)"
...
def main():
    ...
    print("\n=== Improved Physical Tracking Test ===")
    print("This script will track detected objects without squirting.")
